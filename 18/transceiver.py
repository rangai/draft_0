...
class Transceiver:
    ...
    def receive(self, my_port):
        ...
        while True:
            ...
            if msg[0] == SHUTDOWN and msg[1] == my_port:
                soc.close()
                break
            transactions, connected_nodes = self.read_msg(my_port, msg, transactions, connected_nodes)
            soc.close()
        
        r_socket.close()
    
    def read_msg(self, my_port, msg, transactions, connected_nodes):
        ...
        if msg_type == MSG_TX:
            if sender not in connected_nodes:
                ...
                connected_nodes = self.broadcast(node_msg_json, my_port, connected_nodes_list)
            ...
            if tx not in transactions:
                ...
                self.broadcast(tx_json, my_port, connected_nodes)
    ...
    def broadcast(self, msg, sender, nodes):
